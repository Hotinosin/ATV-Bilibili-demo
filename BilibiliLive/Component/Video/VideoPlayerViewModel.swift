...
class VideoPlayerViewModel {
                                      ...
                                      danmuProvider: VideoDanmuProvider) -> [CommonPlayerPlugin]
    {
        ...
        let playlist = VideoPlayListPlugin(sequenceProvider: hasCollection ? collectionProvider : sequenceProvider)
    ...
    @MainActor private func makeCollectionProvider(from episodes: [VideoDetail.Info.UgcSeason.UgcVideoInfo]) -> VideoSequenceProvider? {
        ...
        let sequence = episodes.map {
            PlayInfo(aid: $0.aid, cid: $0.cid, title: $0.title)
        }
        return VideoSequenceProvider(seq: sequence, currentIndex: index)
    }
